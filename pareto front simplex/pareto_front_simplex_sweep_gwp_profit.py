#!/usr/bin/env python3
"""Sweep GWP/profit utility weights on the cost-free simplex edge.

This script keeps the original optimization file untouched and instead runs a
separate design-space exploration where:

- utility cost weight is fixed at 0
- GWP and profit weights span the edge w_gwp + w_profit = 1.0
...
builds a Pareto-front figure for GWP_annual_ops vs Annual_Profit.
"""
...
def simplex_edge(step: float) -> list[tuple[float, float, float]]:
    ...
    for i in range(n + 1):
        w_gwp = i * step
        w_profit = 1.0 - w_gwp
        weights.append((0.0, float(w_gwp), float(w_profit)))
    return weights
...
def plot_pareto_front(rows: list[dict[str, Any]], out_path: Path) -> None:
    finite_rows = [r for r in rows if np.isfinite(r.get("GWP_annual_ops", float("nan"))) and np.isfinite(r.get("Annual_Profit", float("nan")))]
    if not finite_rows:
        raise ValueError("No finite rows available for Pareto plot")

    efficient = _is_pareto_efficient(finite_rows, "GWP_annual_ops", "Annual_Profit")
    ...
    front_rows.sort(key=lambda r: float(r["GWP_annual_ops"]))

    fig, ax = plt.subplots(figsize=(8.5, 6.5))
    gwp = np.array([float(r["GWP_annual_ops"]) for r in finite_rows], dtype=float)
    profit = np.array([float(r["Annual_Profit"]) for r in finite_rows], dtype=float)
    wgwp = np.array([float(r["w_gwp"]) for r in finite_rows], dtype=float)

    sc = ax.scatter(gwp, profit, c=wgwp, cmap="viridis", s=72, edgecolors="black", linewidths=0.35, alpha=0.9, label="Sweep runs")
    if len(front_rows) >= 2:
        fx = np.array([float(r["GWP_annual_ops"]) for r in front_rows], dtype=float)
        fy = np.array([float(r["Annual_Profit"]) for r in front_rows], dtype=float)
        ax.plot(fx, fy, color="#d62728", linewidth=2.2, marker="o", markersize=5, label="Pareto front")

    for row in front_rows:
        ax.annotate(
            f"{row['w_gwp']:.1f}/{row['w_profit']:.1f}",
            (float(row["GWP_annual_ops"]), float(row["Annual_Profit"])),
            xytext=(6, 5),
            textcoords="offset points",
            fontsize=8,
        )

    ax.set_title("Pareto Front: GWP vs Profit Weight Sweep")
    ax.set_xlabel("GWP_annual_ops")
    ax.set_ylabel("Annual_Profit")
    ax.grid(True, alpha=0.25)
    cbar = fig.colorbar(sc, ax=ax, fraction=0.046, pad=0.04)
    cbar.set_label("w_gwp")
...
def _default_output_dir() -> Path:
    ...
    return Path(__file__).resolve().parent / "results" / f"pareto_front_simplex_gwp_profit_{ts}"


def main() -> None:
    parser = argparse.ArgumentParser(description="Sweep the fixed-baseline utility weights along the GWP/profit simplex edge.")
    parser.add_argument("--step", type=float, default=0.1, help="Step size along the weight edge (must divide 1 exactly).")
    parser.add_argument(
        "--out-dir",
        type=str,
        default=None,
        help="Output directory. Defaults to pareto front simplex/results/pareto_front_simplex_gwp_profit_<timestamp>",
    )
    ...
    print(f"Running {len(weights)} GWP/profit weight optimizations with cost weight fixed at 0.0...")
    for idx, weight_triplet in enumerate(weights, start=1):
        ...
        print(
            ...
            f"success={row['run_success']} Utility_Norm={_fmt(row['Utility_Norm'])} GWP={_fmt(row['GWP_annual_ops'])} Profit={_fmt(row['Annual_Profit'])}"
        )
    ...
    out_csv = out_dir / "pareto_front_simplex_gwp_profit_results.csv"
    out_fig = out_dir / "pareto_front_simplex_gwp_profit_figure.png"
