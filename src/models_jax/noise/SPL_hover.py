...
try:
    ...
    try:
        from scipy.special import jv as _scipy_jv  # type: ignore

        def _bessel_jv(v, x):
            # Use SciPy's accurate Bessel implementation when JAX lacks jv.
            return jnp.asarray(_scipy_jv(np.asarray(v), np.asarray(x)))

    except Exception:
        def _bessel_jv(v, x):
            # Last-resort approximation if neither JAX nor SciPy Bessel is available.
            return 1.0 - (x ** 2) / 4.0
...
def tonal_noise_hover(T_req_prop_hover, Power_req_hover, R_prop_hover, rho, n_prop_vert, n_blade_vert, params):
    ...
    n_rot_rpm = hover_rotor_rpm(T_req_prop_hover, R_prop_hover, rho, C_T)
...
def hover_rotor_rpm(T_req_prop_hover, R_prop_hover, rho, C_T):
    """Return hover rotor speed in RPM from thrust coefficient relation."""
