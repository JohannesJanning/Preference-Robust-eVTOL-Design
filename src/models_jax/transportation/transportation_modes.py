...
def transportation_mode_comparison(
    ...
    transportation modes (cars, buses, trains, and
    ...
    # Modes
    modes = [
        "Gasonline Vehicle (20%)", "Diesel Vehicle (20%)",
        "Electric Vehicle (20%)", "Gasonline Vehicle (100%)",
        "Diesel Vehicle (100%)", "Public Bus (100%)", "Electric Vehicle (100%)",
        "Train (100%)", "Diesel Vehicle (26%)",
        "Electric Vehicle (26%)", "Gasonline Vehicle (26%)",
        "Public Bus (60%)", "Train (50%)", "eVTOL",
    ]

    CO2_kg_skm = np.array([
        0.157, 0.128, 0.065, 0.031, 0.026, 0.013, 0.013, 0.007,
        0.099, 0.050, 0.120, 0.022, 0.012, eVTOL_CO2_kg_skm,
    ])
    energy_consumption = np.array([
        632.40, 480.00, 172.70, 126.48, 96.00, 49.42, 34.54, 57.02,
        369.23, 132.85, 486.46, 82.84, 114.04,
        eVTOL_energy_consumption,
    ])
    costs_eur_skm = np.array([
        0.117, 0.083, 0.105, 0.023, 0.017, 0.060, 0.021, 0.200,
        0.064, 0.081, 0.090, 0.104, 0.402, eVTOL_costs_eur_skm,
    ])
    ...
    for i, mode in enumerate(modes):
        results.append({
            ...
            "Time Rating (1–10)": float(time_rating[i]),

            "CO2 (kg)": float(CO2_total[i]),
            "CO2 Rating (1–10)": float(co2_rating[i]),

            "Energy (Wh)": float(energy_total[i]),
            "Energy Rating (1–10)": float(energy_rating[i]),

            "Cost (€)": float(costs_total[i]),
            "Cost Rating (1–10)": float(cost_rating[i]),
        })
