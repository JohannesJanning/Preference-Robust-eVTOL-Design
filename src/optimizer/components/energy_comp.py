...
class EnergyComp(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        eps = 1e-3
        t_climb = climb_time(p.h_cruise, p.h_hover, p.roc_climb_target)
    ...
    def compute_partials(self, inputs, partials):
        ...
        def fun(x):
            ...
            p = self.options['parameters']
            eps = 1e-3
            t_climb = climb_time(self.options['parameters'].h_cruise, self.options['parameters'].h_hover, self.options['parameters'].roc_climb_target)
